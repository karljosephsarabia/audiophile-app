...
const Header = () => {
    ...
    const { setOpenModal, notif, setNotif } = useContext(CartContext);
                ...
                <nav className=" bg-black flex flex-row justify-between px-5 py-8 items-center container mx-auto border-b-2">
                        ...
                        <ul className="flex-row gap-[4vw] font-bold hidden lg:flex ">
                            {menuNav.map((nav, index) =>
                                index < 4 && <li key={index}>
                                    <button className={`uppercase hover:text-orange ${nav.path === location.pathname ? 'text-orange' : 'text-white'}`} onClick={() => navigate(nav.path)}>{nav.title}</button>
                                </li>
                            )}
                        </ul>
                    </div>
                    ...
                    <div className="">
                        <button onClick={handleCartButton}>
                            {
                                notif ? <div className="notif"></div> : null
                            } 
